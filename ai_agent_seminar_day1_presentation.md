...
Skill은 Agent가 반복 작업을 수행할 때 참고하는 절차, 판단 기준, 예시, 산출물 형식을 패키징한 작업 방식입니다.

특정 제품의 기능명이라기보다,
반복 가능한 일의 구조를 만드는 개념입니다.
...
장애 분석 Skill 예시:
- 입력: service, symptom, range
- 절차: Metric → Log → Deploy 순서로 확인
- 출력: 원인 후보, 근거, 조치안
...
response = model.invoke("장애 보고서 초안을 요약해줘")
...
ReAct는 LLM이 답변을 바로 끝내는 방식이 아니라,
필요한 경우 Tool을 호출하고 결과를 다시 판단에 반영하는 반복 구조입니다.

1. **LLM 판단**
   - 사용자 요청과 현재 state를 보고 다음 행동을 결정합니다.
2. **Tool 호출**
   - 외부 정보나 실행이 필요하면 tool_call을 생성합니다.
3. **Tool 결과 반영**
   - Tool 결과를 메시지와 state에 추가합니다.
4. **다시 판단**
   - 추가 Tool이 필요한지, 최종 답변할 수 있는지 다시 판단합니다.
5. **최종 답변**
   - 더 이상 tool_call이 없으면 사용자에게 답변합니다.

---

# ReAct 구성 요소

ReAct 루프는 상태, 모델 판단, Tool 실행, 조건 분기로 구성됩니다.
LangGraph는 이 구성 요소를 graph로 명시하게 해줍니다.

- **State**
  - messages, Tool 결과, 중간 판단을 유지
- **LLM Node**
  - 현재 state를 보고 답변하거나 tool_call을 생성
- **Tool Node**
  - LLM이 요청한 Tool을 실행하고 결과를 저장
- **Conditional Edge**
  - tool_call이 있으면 Tool Node로, 없으면 종료로 분기
- **Stop Condition**
  - 더 이상 tool_call이 없을 때 최종 답변으로 종료
- **Custom Control**
  - 검증, 승인, 재시도, 로그 수집을 graph 단계에 추가

핵심은 API 이름이 아니라,
LLM 판단과 Tool 실행을 상태 기반 graph loop로 만든다는 점입니다.

---

# ReAct 구현 1: create_agent 사용

가장 빠른 시작 방식입니다.
API는 LangChain에 있지만, 내부 실행은 LangGraph 기반 agent runtime에서
model node와 tools node를 반복합니다.

```python
from langchain.agents import create_agent

def get_metric(service: str, metric: str) -> str:
    return f"{service} {metric}: 82%"

agent = create_agent(
    model="openai:<model-name>",
    tools=[get_metric],
    system_prompt="상태 질문은 Tool 결과로 답한다.",
)

agent.invoke({"messages": [{
    "role": "user",
    "content": "checkout CPU 확인",
}]})
```

장점: 코드가 짧고 기본 ReAct loop, Tool 실행, stop condition이 이미 구성되어 있습니다.

---

# ReAct 구현 2: StateGraph 직접 구성

실행 흐름을 직접 제어하고 싶을 때는
State, Node, Conditional Edge를 명시합니다.
...
def call_tools(state):
    ai = state["messages"][-1]
    return {"messages": state["messages"] + run_tools(ai.tool_calls)}

def next_step(state):
    ai = state["messages"][-1]
    return "tools" if ai.tool_calls else END
```
...
장점: 검증, 승인, 재시도, 로그 수집 같은 운영 제어 지점을 graph에 직접 끼워 넣을 수 있습니다.
...
- Agent Card
  - 역할, skill, endpoint, 인증 방식
- Message
  - Agent에게 보내는 요청과 대화 단위
- Task
  - 장기 실행 작업의 상태 추적 단위
- Artifact
  - Agent가 생성해 돌려주는 결과물
...
| 구분 | REST API | A2A |
|---|---|---|
| 호출 대상 | 정해진 기능 endpoint | 독립된 Agent 서비스 |
| 요청 의미 | 파라미터로 기능 실행 | 메시지로 작업 위임 |
| 응답 방식 | 즉시 결과 중심 | Message, Task 상태, Artifact 반환 |
| 기능 설명 | OpenAPI 문서 | Agent Card |
| 핵심 차이 | 기능 호출 | Agent에게 작업을 맡기고 결과를 추적 |

A2A는 독립된 Agent 서비스들이 서로 작업을 위임하고, 상태와 결과를 주고받기 위한 **공통 통신 규약**입니다.

---

# A2A 통신 흐름 예시

A2A는 독립 Agent 서비스가 HTTP 기반 표준 메시지로 작업을 위임하고
결과를 주고받는 구조입니다.

1. 발견
   - Host Agent가 DB Agent Card 조회
   - slow query 분석 skill 확인
   - endpoint, 인증 방식 확인
   - 지원 protocol binding 선택

2. 작업 위임과 결과

```text
message/send
  "checkout slow query 분석해줘"
...
Agent Card는 **누구에게 맡길지**를 정하고,
Message/Task/Artifact는 **어떻게 맡기고 추적하고 결과를 받을지**를 정합니다.
